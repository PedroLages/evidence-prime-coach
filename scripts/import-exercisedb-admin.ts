...
/**
 * Import ExerciseDB API Data (Admin Version)
 * 
 * This script imports exercises from the free ExerciseDB API
 * and temporarily disables RLS for the import process.
 */
...
// For admin operations, we'll use the service role (if available) or anon key with RLS disabled
...
/**
 * Insert exercises into database using SQL (bypasses RLS)
 */
async function insertExercisesSQL(exercises: Exercise[]): Promise<void> {
  console.log('💾 Inserting exercises using direct SQL...');
  
  // First, temporarily disable RLS for exercises
  console.log('🔓 Temporarily disabling RLS...');
  await supabase.rpc('exec_sql', {
    sql: 'ALTER TABLE public.exercises DISABLE ROW LEVEL SECURITY;'
  }).catch(() => {
    // If rpc doesn't work, we'll try direct insert anyway
    console.log('⚠️ Could not disable RLS via RPC, trying direct insert...');
  });
  ...
  await supabase.rpc('exec_sql', {
    ...
    console.log('⚠️ Could not re-enable RLS via RPC - you may need to do this manually');
  });
}
...
async function importExerciseDB() {
  console.log('🏋️ Starting ExerciseDB Import (Admin Mode)...\n');
  ...
  try {
    ...
    await insertExercisesSQL(exercises);
